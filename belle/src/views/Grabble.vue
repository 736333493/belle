<template>
  <div class="Grabble_box">
    <div class="clearfix seek">
      <div class="fl seek_return" @click="goState({ name: 'main' })"><van-icon name="arrow-left" size="22px" /></div>
      <input type="text" class="fl input_text" v-model="value" />
    ...
    <div class="slide">
      <div style="height: 100%;">
        <div
          ...
          @click="goState({ name: 'particular', query: { url: tank.url, tag: tank.tag } })"
        >
          ...
          <div class="fr history-list">
            ...
            <div>类型：{{ tank.tag }}</div>
            <div class="two-tetx">{{ tank.introduce }}</div>
          </div>
        </div>
      </div>
    </div>
